...
Version:	3.0
Created:	12 Aug 2020
Function:	Code below starts off with the IMGT data and filters it down to required resolution
          and then continues with comparing the outfile with the PyIgClassify list and then 
          uses that list to download the required list of interactions from the PDBSum website. 
...
matchingPDB('IGHV1-69_01 Data.csv', 'filtered_IMGT.csv', 'PDBsmatched.csv')
...
def download_file(infile):
    ...
    protein-protein interface' document for each PDB"""
    PDB_searchcodes = []
    ...
    for entry in (df['IMGT entry ID'].str.lower()):
        ...
        filename = req.url[url.rfind('?')+5:-18]
        with open(filename, 'wb') as f:
            for chunk in req.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
                    
download_file('PDBsmatched.csv')

def PDB_bonds(infile, outfile):
...
PDB_bonds('5alb.txt', '5alb_bonds.csv')
